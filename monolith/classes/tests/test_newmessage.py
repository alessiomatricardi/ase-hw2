...
class TestNewMessage(unittest.TestCase):
    ...
    def test_get_list_of_recipients_email(self):
        with self.app.app_context():
            ...
            expected_result = ("user2@example.com", "user2@example.com")
    ...
    def test_create_new_message(self):
        with self.app.app_context():
            ...
            expected_result = db.session.query(Message).filter(Message.id == 11).first() # 11 because there should be 10 default messages in the db
    ...
    def test_email_to_id(self):
        with self.app.app_context():
            result = m.email_to_id("user1@example.com")
            self.assertEqual(1, result)

    def test_create_new_message_recipient(self):
        with self.app.app_context():
            message_recipient = Message_Recipient()
            message_recipient.id = 11 # create recipients for the previously insterted message
            ...
            expected_result = db.session.query(Message_Recipient).filter(Message_Recipient.id == 11).first()
    ...
    def test_send_bottle(self):
        with self.app.app_context():
            message = db.session.query(Message).filter(Message.id == 11).first()
            result = m.send_bottle(message)
            expected_result = db.session.query(Message).filter(Message.id == 11).first().is_sent
    ...
    def test_rendering(self):
        ...
        data = { 'email' : 'user1@example.com' , 'password' : 'user1' } 
        ...
        response = tested_app.get("/new_message?single_recipient=user2@example.com", content_type='html/text', follow_redirects=True)
        self.assertEqual(response.status_code, 200)
        assert b'<input type="checkbox" name="recipients" value=user2@example.com checked>' in response.data
        ...
        response = tested_app.post("/new_message", data = data, content_type='application/x-www-form-urlencoded', follow_redirects=True)
        assert b'Hi user1!' in response.data 
